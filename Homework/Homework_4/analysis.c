...
int main(int argc, char *argv[]){
    ...
    float* costs = (float*)malloc(ITEMS * sizeof(float));
    costs = readCosts(ITEMS);

    float* purchaseCosts = (float*)malloc(ITEMS * sizeof(float));
    purchaseCosts = readPurch(ITEMS);
    
    get_totals(ITEMS, argc, month_data);
    get_profits(ITEMS, argc, month_data, money);
    top_earner(ITEMS, argc, month_data, money);
